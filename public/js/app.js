...
function addNewTweet(tweet) {
  ...
  newTweet.onclick = (e) => (location.href = `/${tweet.user.username}/${tweet.id}`);
  newTweet.innerHTML = `
  <div class="me-3 flex-shrink-0">
    <img src="${tweet.user.profile} " alt="${tweet.user.username + " Avatar"} "
    class="img-fluid user-image" />
  </div>
  <div class="flex-fill">
    <div class="d-flex justify-content-between align-items-center mb-1">
      <a href="/${tweet.user.username}" class="fs-7 fw-bold m-0 text-black"
        ><span class="hover-underline">
          ${tweet.user.firstname + " " + tweet.user.lastname}</span
        ><span class="fw-normal text-secondary ms-1">@${tweet.user.username} &middot; 1m</span>
      </a>
      <div class="dropdown" =>
      <a
      ...
      <div
        class="dropdown-menu dropdown-menu-end rounded-3 shadow bg-white border-0"
        onclick="event.stopPropagation()"
      >
        <form
          action="/${tweet.user.username}/${tweet.id}  "
          method="post"
        ...
        >
          ...
          <button
            type="submit"
            class="w-100 border-0 btn-gray fs-7 text-start p-2 d-flex align-items-center"
            id="delete-btn"
          >
  ...
  <div class="d-flex align-items-center justify-content-between tweet-actions">
    ...
    >
    <!-- LIKES -->
    <a class=" btn-like d-inline-flex align-items-center fs-7" href="/tweet/like/${tweet.id}" >
    <div class="rounded-circle p-2 d-inline-flex justify-content-center align-items-center me-lg-1">
      <svg viewBox="0 0 24 24">
        <g>
          <path
    ...
    <span class="likes-count">${tweet.likes.length}</span>
    <!-- SHARE -->
    <a href="#" class="btn-soft d-inline-flex align-items-center fs-7"
      ><div
        class="rounded-circle p-2 d-inline-flex justify-content-center align-items-center me-lg-1"
